...
def ajuste_tamano1(archivo):
    ...
    N_, M_ = archivo.shape  # Asumimos que `archivo` es una imagen 2D

    # Calcula las nuevas dimensiones que son múltiplos de 32
    N_crop = N_ - (N_ % 32)  # Tamaño de N más cercano hacia abajo que es múltiplo de 32
    M_crop = M_ - (M_ % 32)  # Tamaño de M más cercano hacia abajo que es múltiplo de 32

    # Recorta la imagen
    recortada = archivo[:N_crop, :M_crop]
    return (recortada)
